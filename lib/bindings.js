...
const RadioAccessStatus = Windows.Devices.Radios.RadioAccessStatus;
...
NobleBindings.prototype.init = function() {
	...
	debug('initialized');
	// rt.promisify(Radio.requestAccessAsync)().then(accessLevel => {
	// 	if (accessLevel != RadioAccessStatus.Allowed)
	// 	{
	// 		throw new Error("App is not allowed to control radios: " + accessLevel);
	// 	}
	// 	else
	// 	{
			this._init();
...
NobleBindings.prototype._onAdvertisementWatcherReceived = function(sender, e) {
	...
	if (e.advertisement.manufacturerData && e.advertisement.manufacturerData.length > 0) {
		debug('manufacturerData', deviceRecord.name, e.advertisement.manufacturerData, e.advertisement.manufacturerData.length);
		let manufacturerDataSection = e.advertisement.manufacturerData[0];
		debug('manufacturerData2', deviceRecord.name,manufacturerDataSection, manufacturerDataSection.data.length, manufacturerDataSection.data[0]);
		deviceRecord.manufacturerData =  rt.toBuffer(manufacturerDataSection.data);
		debug('manufacturerData3', deviceRecord.name, deviceRecord.manufacturerData.length);
	}
	...
	if (e.advertisementType == BluetoothLEAdvertisementType.scanResponse) {
		...
		debug('scanResponse', advertisement);
