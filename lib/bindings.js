...
const bth = require("bth-connectable");
...
const Radios = Windows.Devices.Radios;
...
NobleBindings.prototype._updateRadioState = function () {
	...
	bth.enableConnections();
	const connectable = bth.connectable();
...
NobleBindings.prototype._onAdvertisementWatcherReceived = function (sender, e) {
	...
	// if (e.advertisement.localName.indexOf("Genius") !== -1) {
	// 	console.log('watcher received: %s %s %s',
	...
	dataSections.forEach(dataSection => {
		...
		switch (dataSection.dataType) {
			...
			case BluetoothLEAdvertisementDataTypes.completeService128BitUuids:
				...
				let index = serviceDataEntry.uuid.length - 2;
				let ordered = ""
				while(index >= 0) {
					ordered = ordered + serviceDataEntry.uuid[index] + serviceDataEntry.uuid[index+1];
					index = index - 2;
				}
				serviceDataEntry.uuid = ordered;
				break;
	...
	if (!this._acceptOnlyScanResponse || isScanResponse) {
		...
		if (deviceRecord.serviceUuids) {
			for (let index in deviceRecord.serviceUuids) {
				const service_id = deviceRecord.serviceUuids[index];
				if (this._filterAdvertisementServiceUuids.indexOf(service_id) >= 0) {
					serviceUuidMatched = true;
				}
			}
		}
		if (!serviceUuidMatched) {
			for (let id in serviceDataEntry) {
				if (this._filterAdvertisementServiceUuids.indexOf(serviceDataEntry[id]) >= 0) {
					serviceUuidMatched = true;
				}
			}
		}
		...
		if (serviceUuids) {
			joinedIds.push(...serviceUuids);
		}
		for (let id in serviceDataEntry) {
			joinedIds.push(serviceDataEntry[id]);
		}
		...
		debug('discover: %s [%s]', deviceUuid, joinedIds.join());
		this.emit(
			'discover',
			deviceUuid,
			address,
